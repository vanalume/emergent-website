...
The provider stays fully dormant until SHIPROCKET_EMAIL, SHIPROCKET_PASSWORD and
SHIPROCKET_PICKUP_LOCATION are set — is_configured() will return False and every
call is a no-op/error path. This mirrors the Razorpay approach in config.py.
"""
...
class ShipRocketProvider(DeliveryProvider):
    ...
    @property
    def id(self) -> str:
        return self.config.get("id", "shiprocket")
    ...
    def is_configured(self) -> bool:
        return bool(
            self._env("SHIPROCKET_EMAIL")
            and self._env("SHIPROCKET_PASSWORD")
            and self._env("SHIPROCKET_PICKUP_LOCATION")
        )

    async def _login(self, client: httpx.AsyncClient) -> str:
        r = await client.post(
            f"{SR_BASE}/auth/login",
            json={"email": self._env("SHIPROCKET_EMAIL"), "password": self._env("SHIPROCKET_PASSWORD")},
            timeout=20,
        )
        r.raise_for_status()
        data = r.json()
        ...
        self._token_expiry = time.time() + 9 * 24 * 3600
        return token

    async def _token_valid(self, client: httpx.AsyncClient, force: bool = False) -> str:
        if self._token and time.time() < self._token_expiry and not force:
            return self._token
        return await self._login(client)

    async def _request(self, method: str, path: str, **kwargs) -> Any:
        async with httpx.AsyncClient(timeout=30) as client:
            token = await self._token_valid(client)
            headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
            r = await client.request(method, f"{SR_BASE}{path}", headers=headers, **kwargs)
            if r.status_code == 401:
                token = await self._token_valid(client, force=True)
    ...
    # -- fulfillment ------------------------------------------------------
    def _build_adhoc_payload(self, order_doc: dict) -> dict:
        ...
        return {
            ...
            "pickup_location": self._env("SHIPROCKET_PICKUP_LOCATION"),
            "channel_id": "",
    ...
    async def create_adhoc_order(self, order_doc: dict) -> Optional[dict]:
        ...
        if not self.is_configured():
            return None
        try:
            payload = self._build_adhoc_payload(order_doc)
    ...
    async def track_by_source_order(self, source_order_id: str) -> dict:
        return await self._request("GET", "/courier/track", params={"order_id": source_order_id})
