...
- **Bar chart** — top N products by units (toggle: units ↔ revenue) - N configurable by the admin user
- **Table** below (grouped by category or product, toggle) with sortable columns

**Chart choice rationale:**
- *Line* for trend over time (revenue, orders) - Approved
- *Stacked bar* is an option if you want revenue-by-category over time; can add later - Add now
- *Donut* for revenue mix by category (clear at-a-glance share) - Approved
- *Horizontal bar* for top-N products (labels readable) - Approved, but make N configurable from the admin dashboard 
...
1. **Storage:** Supabase (needs your keys + adds vendor lock-in)?
2. **Bonus Phase 7 (Admin Orders):** defer
3. **Auth on `/admin`:**  Single key is fine for now
4. **Offer stacking rule:** confirm "higher discount wins" when two active offers overlap a category.
5. **Sales data cut-off:** include, since the money's in.
6. **CMS scope:** Contact intro + Footer + Navbar links editable too from day one
